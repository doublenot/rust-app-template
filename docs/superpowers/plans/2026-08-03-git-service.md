...
| task | finding | fix |
|---|---|---|
| 10 | Task 2's last `#[allow(dead_code)]` — on `git_enabled()`. The four task 9 owned are gone; `#[allow]` does not error when it stops firing, so this one rots silently until its reader lands | Delete it when `host::App::start_children` calls `git_enabled()` |
